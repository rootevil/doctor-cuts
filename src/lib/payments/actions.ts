...
/** Logged-in customer retry — ownership checked against the session. */
export async function retryDepositCheckoutForUser(input: {
  ...
  const ipRl = await limitByIp("retryDepositUser", 5, 60 * 60_000);
  const userRl = await limitByKey(
    "retryDepositUser",
    user.id,
    8,
    60 * 60_000,
  );
  if (!ipRl.ok || !userRl.ok) return { ok: false, reason: "rate_limited" };
